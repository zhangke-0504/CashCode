...
`SimpleDream` 负责在后台周期性地将各 chat_id 中新积累的对话历史，通过两次 LLM 调用汇总更新到全局 `memory/MEMORY.md`，形成 Agent 的跨会话长期记忆；并以 `memory/.dream_cursor` 记录各 chat_id 的处理进度，实现增量处理与故障情况下的安全退出。
